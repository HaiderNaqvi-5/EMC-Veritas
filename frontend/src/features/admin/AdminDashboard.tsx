...
export function AdminDashboard() {
  ...
  const activeStudents = useMemo(() => students.data?.filter((item) => item.active).length ?? 0, [students.data]);
  const activeSessions = useMemo(() => sessions.data?.filter((item) => item.status === "ACTIVE").length ?? 0, [sessions.data]);
  const publishedActivities = useMemo(() => activities.data?.filter((item) => item.status === "PUBLISHED").length ?? 0, [activities.data]);
  const chart = useMemo(() => ["DRAFT", "READY", "PUBLISHED", "ARCHIVED"].map((status) => ({ status, count: activities.data?.filter((item) => item.status === status).length ?? 0 })), [activities.data]);
  const cards = useMemo(() => [["Active session", activeSessions], ["Active students", activeStudents], ["Published activities", publishedActivities], ["All activities", activities.data?.length ?? 0]], [activeSessions, activeStudents, publishedActivities, activities.data]);
